...
def prefilter_and_candidates(session, *, ingredient_ids: Optional[List[str]], cuisine: Optional[str],
    ...
    def compute_popularity_score(pop: Dict[str, float]) -> float:
        def lp(x: Optional[float]) -> float:
            try:
                ...
                return 0.0
        views = lp(pop.get("views"))
        saves = lp(pop.get("saves"))
        cooks = lp(pop.get("cooks"))
        likes = lp(pop.get("likes"))
        rating_val = float(pop.get("rating_value") or 0.0)
        rating_cnt = lp(pop.get("rating_count"))
        # ✅ Combine popularity factors and rating value
        score = (
            0.4 * views +
            0.3 * saves +
            0.4 * cooks +
        ...
        return score / 10.0  # normalize to keep scores balanced

    # ✅ Main branch: query recipes matching user-provided ingredients
    if ingredient_ids:
        ...
        q = (
            ...
            // ✅ Compute how many ingredients matched + sum of ingredient rarity (IDF)
            WITH r,
                 collect(i.ingredient_id) AS matched,
                 count(i) AS matchCount,
            ...
            RETURN r.recipe_id AS id,
                   ...
                   pop AS pop,
                   coalesce(r.updated_at, r.created_at) AS updated_at
            ORDER BY (matchCount + rarityScore) DESC, coalesce(pop.views,0) DESC
            LIMIT $lim
            """
        )

        rows = session.run(q, ingIds=ingIds, cuisine=cuisine,
                           maxCook=max_cook_time, lim=limit_candidates)
        recipes = list(rows)
        print(f"✅ Found {len(recipes)} recipes with ingredient overlap.")
        ...
        for row in recipes:
            ...
            # ✅ Ingredient score = combine match count + ingredient rarity
    ...
    # ⚠️ Fallback: if no recipes matched by ingredients, use top popular ones
    if not out:
        print("⚠️ No ingredient matches found — falling back to popular recipes.")
        q = (
            """
            MATCH (r:Recipe)
            WHERE ( $cuisine IS NULL OR r.cuisine = $cuisine )
    ...
    print(f"🚀 Total collected: {len(out)} unique recipes.")
    return out
