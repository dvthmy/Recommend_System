...
def build_recipe_props(row: Dict[str, str]) -> Dict:
    ...
    def parse_calories(x: Optional[str]) -> Optional[float]:
        if x is None:
            return None
        s = str(x).strip().lower()
        # extract first number, allow decimals
        m = re.search(r"(\d+(?:\.\d+)?)", s)
        try:
    ...
    def first_cuisine_token(x: Optional[str]) -> Optional[str]:
        if not x:
            return None
        # keep original casing; take first token before comma
        token = str(x).split(",")[0].strip()
    ...
    calories = parse_calories(row.get("calories"))
    rating_avg = to_float(row.get("rating_value"))
    # prefer rating_count column; fallback to review_count
    rating_cnt = None
    try:
        rating_cnt = int(re.search(r"(\d+)", str(row.get("rating_count") or "")).group(1)) if row.get("rating_count") else None
    except Exception:
        rating_cnt = None
    if rating_cnt is None and row.get("review_count"):
        try:
            rating_cnt = int(re.search(r"(\d+)", str(row.get("review_count"))).group(1))
        except Exception:
            rating_cnt = None
