...
def _find_cols(header: List[str]) -> Dict[str, int | List[int]]:
    ...
    # ---- Các cột cơ bản
    name_idx   = find_one(["hovaten", "fullname", "hoten", "hovat", "full name", "ho va ten"])
...
def import_survey(csv_path: str, uri: str, user: str, password: str, database: Optional[str]):
    ...
    try:
        with open(csv_path, "r", encoding="utf-8-sig") as f:
            ...
            if cols["name"] != -1:
                print(f"✅ Found name column at index {cols['name']}: '{header[cols['name']]}'")
            else:
                print(f"⚠️  Name column not found. Available headers: {header[:5]}...")
    ...
    print(f"✅ Imported users={created_users}, allergies={linked_allergies}, cuisines={linked_fav_cuisines}, recipes={linked_recipes}")
